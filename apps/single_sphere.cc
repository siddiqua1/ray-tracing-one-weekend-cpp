...
bool hit_sphere(const point3 &center, f64 radius, const ray &r) {
  const auto C_minus_Q = center - r.origin();
  const auto dir = r.direction();
  const auto a = dot(dir, dir);
  const auto b = -2.0 * dot(dir, C_minus_Q);
...
color ray_color(const ray &r) {
  ...
  const auto sphere_color = color(1, 0, 0);
...
int main() {
  ...
  auto file = utils::open<'w'>("single_sphere.ppm");
  file.write_line("P3");
