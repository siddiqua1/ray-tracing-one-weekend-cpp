...
static std::tuple<int, int, int> to_tuple(const color &pixel_color) {
...
inline void write_color(std::ostream &out, const color &pixel_color) {
  ...
  auto [red_byte, green_byte, blue_byte] = to_tuple(pixel_color);
...
inline void write_color(std::vector<std::tuple<int, int, int>> &stream,
  ...
  auto rgb = to_tuple(pixel_color);

  // Write out the pixel color components.
  stream.push_back(rgb);
}

inline void write_color(std::vector<std::tuple<int, int, int>> &stream,
                        const color &pixel_color, size_t idx) {
  // Write out the pixel color components.
  stream[idx] = to_tuple(pixel_color);
}
